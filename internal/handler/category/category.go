...
func NewCategoryHandler(svc categoryservice.CategoryService) *CategoryHandler {
...
func writeCategoryResponse(c *gin.Context, data any, err error) {
	...
	if errors.Is(err, categoryservice.ErrCategoryNotFound) || errors.Is(err, categoryservice.ErrCategoryArticleMissing) {
		response.NotFound(c)
		return
	}
