...
func (a *commentAdapter) Descendants(ctx context.Context, tx *gorm.DB, ref SubjectRef) ([]SubjectRef, error) {
	...
	case SubjectArticleComment:
		...
		for _, row := range rows {
			refs = append(refs, SubjectRef{Type: SubjectArticleCommentReply, ID: uint64(row.ID), RootID: ref.ID, ParentID: uint64(row.ParentReplyID)})
		}
	case SubjectMomentComment:
		...
		for _, row := range rows {
			refs = append(refs, SubjectRef{Type: SubjectMomentCommentReply, ID: uint64(row.ID), RootID: ref.ID, ParentID: uint64(row.ParentReplyID)})
		}
	case SubjectArticleCommentReply, SubjectMomentCommentReply:
...
func replySnapshot(subjectType SubjectType, id, commentID, parentID, authorID uint, content string) SubjectSnapshot {
	return SubjectSnapshot{
		Ref:      SubjectRef{Type: subjectType, ID: uint64(id), RootID: uint64(commentID), ParentID: uint64(parentID)},
